...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    for (const auto& t:TriangleList)
    {
        ...
        rasterize_ssaa_triangle(newtri, viewspace_pos);
    }
}
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos)
{
    ...
    auto v = t.toVector4();
    Eigen::Vector4f vs[3];
    std::transform(std::begin(t.v), std::end(t.v), vs, [](auto& vec) { return Eigen::Vector4f(vec.x(), vec.y(), vec.z(), 1.f); });
    ...
    for(int x = left; x <= right; x++) {
        for(int y = bottom; y <= top; y++) {
            if(insideTriangle(x, y, vs)) {
                auto[alpha, beta, gamma] = computeBarycentric2D(x, y, vs);
                ...
                if(depth_buf[index] > depth) {
                    ...
                    // auto interpolated_shadingcoords 阴影坐标
...
void rst::rasterizer::rasterize_ssaa_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) {
    auto v = t.toVector4();
        ...
        for(int y = 2 * bottom; y <= 2 * top; y++) {
            ...
            if(insideTriangle(real_x, real_y, t.v)) {
                ...
                if(ssaa_depth_buf[index] > depth) {
                    ...
                    Eigen::Vector2i point = Eigen::Vector2i(x, y);
                    // set_pixel(point, pixel_color);
                    set_ssaa_pixel(point, pixel_color);
                }
            }
        }
    }
...
void rst::rasterizer::set_ssaa_pixel(const Vector2i &point, const Eigen::Vector3f& color)
{
